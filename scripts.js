...
// Função para atualizar o rodapé da tabela de relatórios
function atualizarRodapeRelatorio() {
    const tfoot = document.querySelector('#tabelaRelatorio tfoot');
    tfoot.innerHTML = '';
    const tfootRow = document.createElement('tr');
    ...
    document.querySelectorAll('#tabelaRelatorio tbody tr').forEach(row => {
        ...
        colunasSelecionadas.forEach((coluna, index) => {
            if (coluna === 'valorVenda') {
                totalValorVenda += parseFloat(cells[index].textContent.replace('R$', '').replace(',', '.')) || 0;
            } else if (coluna === 'valorBrutoReceber') {
                totalValorBruto += parseFloat(cells[index].textContent.replace('R$', '').replace(',', '.')) || 0;
            } else if (coluna === 'comissao') {
                totalComissao += parseFloat(cells[index].textContent.replace('R$', '').replace(',', '.')) || 0;
            }
        });
    });
